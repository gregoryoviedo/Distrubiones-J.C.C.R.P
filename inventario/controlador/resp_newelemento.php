<?php
    ...
    if(empty($errores)){
        ...
        $ok = $obj->new_elemento($id_categoria,$nombre,$cantidad,$precio,$descripcion);
    
        if($ok){
            $datos['exito'] = true;
            $datos['mensaje'] = "El registro se ha realizado correctamente";
            $auditor->auditar($_SESSION['id_empleado'],$_SESSION['usuario'],'Insert','Inventario','Nuevo elemento: '.$nombre);      
        }
        else{
            $datos['exito'] = false;
