<?php
    ...
    if(empty($errores)){
        ...
        $ok = $obj->hab_ruta($id_ruta,$id_vendedor1,$id_vendedor2,$id_vehiculo);
    
        if($ok){
            $datos['exito'] = true;
            $datos['mensaje'] = "Se ha habilitado correctamente";
            $auditor->auditar($_SESSION['id_empleado'],$_SESSION['usuario'],'Update','Rutas','Cambiar estado ruta ID: '.$id_ruta.' A: habilitado');        
        }
        else{
            $datos['exito'] = false;
